...
async function fetchRekuData() {
  ...
  try {
    ...
    console.error("Reku Error:", error);
  ...
  try {
    ...
    return Object.entries(tickers)
      ...
      .map(([pair, value]) => ({
        ...
        indodax:
          Number(value.vol_idr || 0) /
          1000000000,
      }));

  } catch (error) {

    console.error("Indodax Error:", error);
...
async function fetchTokocryptoData() {
  ...
  try {
    ...
    console.error("Tokocrypto Error:", error);
...
function mergeMarketData(
  ...
  return rekuData.map((rekuCoin) => {

    const indoMatch =
      indodaxData.find(
        (item) =>
          item.asset === rekuCoin.asset
      );

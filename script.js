...
function calculateAction(indodax, reku, tokocrypto) {
  ...
  const betaPos =
    0.7 + (1 - 0.7) * (1 / (1 + Math.exp(-0.015 * (d - 350))));

  const alphaPos =
    0.007 * (1 - 1 / (1 + Math.exp(-0.015 * (d - 350))));

  const betaNeg =
    0.007 * (1 - 1 / (1 + Math.exp(0.1 * (d + 150))));

  const alphaNeg =
    0.7 + (0.9 - 0.7) * (1 / (1 + Math.exp(0.1 * (d + 150))));
...
function mergeMarketData(reku, indodax, tokocrypto) {
  return reku.map((rekuCoin) => {
    ...
    return {
      ...
      reku: rekuCoin.reku || 0,
      indodax: indo.indodax || 0,
      tokocrypto: toko.tokocrypto || 0,
    };
  });
}
    ...
    .map((item) => {
      ...
      return `
        <tr>
          ...
          <td>${formatVolume(item.indodax)}</td>
          <td>${formatVolume(item.reku)}</td>
          <td>${formatVolume(item.tokocrypto)}</td>
...
function renderRefreshTimes() {
  ...
  elements.refreshTimes.reku.textContent = now;
  elements.refreshTimes.tokocrypto.textContent = now;
}
